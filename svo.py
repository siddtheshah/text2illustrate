...
def getObjsFromPrepositions(deps):
    objs = []
    for dep in deps:
        # print(dep, dep.pos_, dep.dep_, list(dep.rights)[0].dep_)
        ...
        if len(subs) > 0:
            ...
            for sub in subs:
                for obj in objs:
                    ...
                    if not (verbNegated or objNegated):
                        svos.append((sub.lower_, v.lower_ , obj.lower_))
    return svos

def findSVOsAsTokens(tokens):
    svos = []
    verbs = [tok for tok in tokens if tok.pos_ == "VERB" and tok.dep_ != "aux"]
    for v in verbs:
        subs, verbNegated = getAllSubs(v)
...
def testSVOs():
    ...
    tok = nlp("He shot at the cow.")
    svos = findSVOs(tok)
    printDeps(tok)
    print(svos)

    print("-----------------------------------------------------")
    tok = nlp("He was shot by the cow.")
    svos = findSVOs(tok)
    printDeps(tok)
    print(svos)
