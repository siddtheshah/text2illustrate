...
class StanfordCoreNLP:
    def __init__(self, path_or_host, port=None, memory='4g', lang='en', timeout=60, quiet=True,
    ...
    def coref(self, text):
        r_dict = self._request('coref -coref.algorithm neural', text)
    ...
    def openie(self, text):
        r_dict = self._request('openie', text)
        fullSROs = []
        for k, sentence in enumerate(r_dict['sentences']):
            sros = []
            for sro_dict in sentence['openie']:
                subSpan = sro_dict['subjectSpan']
                objSpan = sro_dict['objectSpan']
                subTup = (sro_dict['subject'], k+1, subSpan[0] + 1, subSpan[1] + 1)
                objTup = (sro_dict['object'], k+1, objSpan[0] + 1, objSpan[1] + 1)
                sros.append((subTup, sro_dict['relation'], objTup))
            fullSROs.append(sros)
        return fullSROs
    ...
    def get_raw(self, annotator, text):
        r_dict = self._request(annotator, text)
        return r_dict
